...
for i in c:
    for j in gamma: 
        ...
        scores = cross_val_score(clf_svm, pca_X_train_20, y_train.values.ravel())
        score = scores.mean()
...
pca = PCA(n_components=2)
pca_X_train_30 = pca.fit_transform(X_train_30)
...
for i in c:
    for j in gamma:
        clf_svm = SVC(C = i, gamma=j)
        scores = cross_val_score(clf_svm, pca_X_train_30, y_train.values.ravel())
        score = scores.mean()
        scores_mean.append(score)
        if score > max_score:
            max_score = score
...
pca = PCA(n_components=2)
pca_X_train_40 = pca.fit_transform(X_train_40)
...
for i in c:
    for j in gamma:
        clf_svm = SVC(C = i, gamma=j)
        scores = cross_val_score(clf_svm, pca_X_train_40, y_train.values.ravel())
        score = scores.mean()
        scores_mean.append(score)
        if score > max_score:
            max_score = score
